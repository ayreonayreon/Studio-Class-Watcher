...
def login_via_cookies(context, page) -> bool:
    ...
    # Si la sesion expiro, deberiamos ver el formulario de login (campo de
    # password) en vez del calendario. Puede estar en la pagina principal
    # o dentro del iframe, revisamos ambos por seguridad.
    password_in_main = page.locator("input[type='password']").count()
    password_in_frame = 0
    try:
        frame = get_booking_frame(page, timeout_ms=5000)
        password_in_frame = frame.locator("input[type='password']").count()
    except Exception:
        pass
    still_logged_in = (password_in_main + password_in_frame) == 0
