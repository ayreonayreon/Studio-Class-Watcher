...
() => {
  ...
  const startDateStr = startDateInput ? startDateInput.value : null;

  const cards = Array.from(document.querySelectorAll('div.exercise'));
  return cards
    .filter(card => card.innerText.includes('5:45 AM'))
    .map(card => {
      const nameEl = card.querySelector('.class-type.class_name');
      const spotsEl = card.querySelector('.spots-info-text');

      // Las clases LLENAS (addwaitlist) traen data-classdate; las
      // DISPONIBLES (bookspot/showspot) no lo traen -- para esas,
      // calculamos la fecha a partir de la posicion de su columna
      // dentro de la cuadricula (0 = startDate, 1 = startDate+1, etc.)
      let dateStr = card.getAttribute('data-classdate');
      if (!dateStr && startDateStr) {
        const cell = card.closest('.table-cell');
...
def parse_availability(card: dict) -> bool:
    """
    Determina si una tarjeta tiene cupo disponible, confirmado con
    ejemplos reales del sitio:

    DISPONIBLE:
      onclick="showspot(...)" o "bookspot(...)"
      <span class="spots-info-text text-success">Quedan: N</span>
      (OJO: estas tarjetas NO traen data-classdate/data-classtime)

    LLENA (con lista de espera):
      onclick="addwaitlist(...)"  data-autobook="1"
      <span class="spots-info-text text-warning">Quedan: 0</span>
      (estas SI traen data-classdate/data-classtime)
    """
...
def scrape_classes(page) -> dict:
    ...
    clases de 5:45 AM encontradas.

    Filtramos por el TEXTO visible "5:45 AM" en cada tarjeta, no por
    atributos data-classtime/data-classdate -- confirmamos que el sitio
    SOLO agrega esos atributos a las clases LLENAS (onclick=addwaitlist);
    las disponibles (bookspot/showspot) no los traen en absoluto, lo cual
    hacia que el filtro anterior nunca pudiera encontrar cupos reales.
    ...
    # sigue seleccionada (deberia venir asi desde la sesion guardada).
    ...
    for week_index in range(weeks_to_check):
        ...
        print(f"[debug] Semana {week_index}: {len(found)} tarjeta(s) de 5:45 AM encontradas (lectura estable).")
        for card in found:
            print(
...
SESSION_EXPIRED_KEY = "_session_expired_notified_at"
SESSION_EXPIRED_COOLDOWN_HOURS = 6


def should_notify_session_expired() -> bool:
    """
    Evita mandar el aviso de 'sesion expirada' cada 5 minutos sin parar.
    Solo notifica si nunca se habia avisado, o si ya pasaron varias horas
    desde el ultimo aviso (por si la persona no lo vio de inmediato).
    """
    state = load_previous_state()
    last_notified = state.get(SESSION_EXPIRED_KEY)
    ...
    return hours_since >= SESSION_EXPIRED_COOLDOWN_HOURS


def mark_session_expired_notified() -> None:
    state = load_previous_state()
    state[SESSION_EXPIRED_KEY] = datetime.now(timezone.utc).isoformat()
    save_state(state)


def login_and_get_page(playwright):
    browser = playwright.chromium.launch(headless=True)
    # Ventana ancha: sospechamos que las columnas de dias que quedan fuera
    # de la vista (sin scroll horizontal) no reciben sus atributos de datos
    # hasta que entran en pantalla. Con un viewport ancho, las 7 columnas
    # deberian caber sin necesidad de scroll.
    ...
    if EZFIT_COOKIES_JSON:
        ...
        if not session_ok:
            ...
            if should_notify_session_expired():
                send_telegram_message(
                    ...
                    f"(No te voy a volver a avisar hasta dentro de "
                    f"{SESSION_EXPIRED_COOLDOWN_HOURS}h para no saturarte.)"
                )
                mark_session_expired_notified()
            else:
                print("[info] Sesion sigue expirada, pero el aviso esta en cooldown -- no se reenvia.")
            browser.close()
            sys.exit(1)
        return browser, page
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Diagnostico: capturamos cualquier peticion de red hacia ezfit.io
        # que falle (status >= 400) o cualquier error de red, para
        # confirmar si la llamada que trae los datos de "siguiente semana"
        # esta fallando silenciosamente (ej. token de autorizacion
        # distinto al que usamos para cargar la pagina inicial).
        network_issues = []

        def on_response(response):
            if "ezfit" in response.url and response.status >= 400:
                network_issues.append(f"{response.status} {response.url}")

        def on_request_failed(request):
            if "ezfit" in request.url:
                network_issues.append(f"FAILED {request.url} ({request.failure})")
        ...
        try:
            ...
            if network_issues:
                print(f"[debug] Peticiones de red con problemas hacia ezfit.io ({len(network_issues)}):")
                for issue in network_issues[:20]:
                    print(f"[debug]   {issue}")
            else:
                print("[debug] No se detectaron peticiones fallidas hacia ezfit.io.")
            browser.close()
    ...
    # Ordenar por fecha (class_date viene como 'YYYY-MM-DD', asi que el
    # orden alfabetico ya es orden cronologico). Las que no tengan fecha
    # se van al final.
    new_findings.sort(key=lambda f: f.get("class_date") or "9999-99-99")
