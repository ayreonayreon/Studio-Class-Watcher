...
def wait_for_grid_data(frame, page, timeout_seconds: float = 15.0) -> bool:
    """
    El calendario siempre renderiza una cuadricula fija de celdas (~63,
    7 dias x 9 franjas), pero los atributos data-classtime/data-classdate
    de cada celda se rellenan un poco DESPUES, via JS/AJAX. Si leemos antes
    de que eso termine, encontramos celdas sin ningun dato aunque la
    cuadricula ya se vea "llena" a simple vista. Aqui esperamos a que
    exista AL MENOS UNA celda con data-classtime poblado (de cualquier
    hora, no solo 5:45) como senal de que los datos ya cargaron.
    """
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        populated = frame.evaluate(
            "document.querySelectorAll('div.exercise[data-classtime]').length"
        )
        if populated > 0:
            return True
...
def scrape_classes(page) -> dict:
    ...
    for week_index in range(weeks_to_check):
        ...
        data_ready = wait_for_grid_data(frame, page)
        if not data_ready:
            print(f"[warn] Semana {week_index}: los datos de la cuadricula nunca se poblaron tras 15s de espera.")
